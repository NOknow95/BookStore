		...
		<div id="book">
			<div class="book_cond">
				...
				<form action="client/BookClientServlet" method="get">
					<input type="hidden" name="method" value="findBookByPrice" /> 价格：<input
						type="text" name="min" value="${param.min}"> 元 - <input
						value="${param.max }" type="text" name="max"> 元 <input
				...
				<c:otherwise>
					<c:forEach items="${page.data }" var="book">
						<div class="b_list">
							...
							<div class="book_info">
								<div class="book_name">
									<span class="sp1">书名:</span> <span class="sp2">${book.title
										}</span>
								</div>
								<div class="book_author">
									<span class="sp1">作者:</span> <span class="sp2">${book.author
										}</span>
								</div>
								<div class="book_price">
									<span class="sp1">价格:</span> <span class="sp2">￥${book.price
										}</span>
								</div>
								<div class="book_sales">
									<span class="sp1">销量:</span> <span class="sp2">${book.sales
										}</span>
								</div>
								<div class="book_amount">
									<span class="sp1">库存:</span> <span class="sp2">${book.stock
										}</span>
								</div>
								<div class="book_add">
									<button id="${book.id }" class="addCart">加入购物车</button>
									<%-- <a style="color:blue"
										href="client/CartServlet?method=add2Cart&bookId=${book.id }">加入购物车</a> --%>
								</div>
							</div>
						</div>
					</c:forEach>
